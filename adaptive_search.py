...
from adaptive_path import path_opt
...
@njit(cache=True)
def sweep(grid,deficits,ts,zs,alpha_stride=1):
    N=len(grid)-1
    M=len(deficits)+1
    ...
    for i in range(1,N+1):
        t=grid[i];et=eval_profile(ts,zs,t)
        uncond[i]=et
        for j in range(M):
            ...
            if j==M-1 or z>uncond[i]+2e-8:
                ...
                for r in range(i,1,-1):
                    ...
                    if ur>uncond[r-1]+2e-8:
                        ...
                        break
                if bg<0:
                    bg,bm,bi=path_opt(t,z,path_u,first,i,grid,ts,zs)
            ...
            length[i]=insert(i,j,Z[i,j]+(grid[i]-grid[i-1])*Cost[i,j]+1e-8,Cost[i,j],req,co,ids,length[i])
    return Z,R,Cost,A,Mu,FC,startu,depth,req,co,ids,length

@njit(cache=True)
def forward(grid,Z,R,Cost,ts,zs,start):
    N=len(grid)-1
    F=np.full(N+1,1e90);which=np.full(N+1,-1,np.int32)
    j0=max(1,np.searchsorted(grid,start));F[j0]=eval_profile(ts,zs,grid[j0])+1e-7
    for i in range(j0+1,N+1):
        ...
        for j in range(Z.shape[1]):
            if Z[i,j]+1e-8<F[i-1] and Cost[i,j]<best:
                best=Cost[i,j];bj=j
        if bj<0:break
        F[i]=F[i-1]+(grid[i]-grid[i-1])*best;which[i]=bj
    return F,which
