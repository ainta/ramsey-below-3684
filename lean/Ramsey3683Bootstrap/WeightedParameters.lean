...
theorem tendsto_weightedBookParameter {p μ θ : ℝ}
  ...
  change Tendsto (fun r => bookParameter p μ r * (1 - μ) ^ (θ - 1)) _ _
...
theorem weightedProfile_eq_bookProfile
  ...
  have he : θ⁻¹ * (1 - (1 - θ * ρ)) = ρ := by field_simp; ring
  unfold bookProfile
...
theorem weightedProfile_le {x μ θ ρ β u v : ℝ}
  ...
  have he : θ⁻¹ * (θ * ρ) = ρ := by field_simp
  have he' : 1 - (1 - θ * ρ) = θ * ρ := by ring
  simpa only [he', ← Real.rpow_mul hx.le, he] using h
...
theorem weighted_root_gap {x p μ θ : ℝ} {r : ℕ}
  ...
  have hpow' : x ^ ((r : ℝ)⁻¹) < A * B ^ (θ * (r : ℝ)⁻¹ - 1) := by
    calc
      ...
      _ = (A ^ r) ^ ((r : ℝ)⁻¹) *
          (B ^ (θ - (r : ℝ))) ^ ((r : ℝ)⁻¹) :=
        Real.mul_rpow (pow_nonneg hA.le _) (Real.rpow_nonneg hB.le _)
      _ = A * B ^ (θ * (r : ℝ)⁻¹ - 1) := by
